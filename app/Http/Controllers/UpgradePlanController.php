...
class upgradePlanController extends Controller
{
    ...
    protected function FlutterwavePayment(Request $request)
    {
        ...
        $data = [
            ...
            'callback_url' => $baseURL . '/app/payment/success?gateway=flutterwave',
            'metadata' => [
                'plan_id' => $plan_id,
            ],
        ];
        ...
        if ($response->successful() && isset($responseBody['status'])) {
            if ($responseBody['status'] === 'success') {
                ...
                return response()->json(['error' => __('messages.payment_initialization_failed') . ($responseBody['message'] ?? __('messages.unknown_error'))],400);;
