...
class BranchController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('backend.branch.index_datatable', compact('module_action', 'filter', 'select_data', 'assets', 'columns', 'customefield','module_title'));
    }
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $query = Branch::withCount('branchEmployee')->with('media', 'address', 'employee')->where('created_by', auth()->id());
        ...
        $datatable = $datatable->eloquent($query)
            ...
            ->orderColumn('manager_id', function ($query, $order) {
                $query->select('branches.*')
                    ->leftJoin('users', 'users.id', '=', 'branches.manager_id')
