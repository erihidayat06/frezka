         ...
         <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="mb-2 navbar-nav ms-auto align-items-center navbar-list mb-lg-0">
              <li class="nav-item dropdown me-0 me-xl-3">
                <div class="d-md-flex d-none align-items-center me-2 iq-font-style" role="group" aria-label="First group">
                    ...
                    <div class="m-0 shadow-none card">
                      ...
                      <div class="p-0 card-body max-17 scroll-thin">
                        <form action="{{ route('backend.reset-branch') }}" method="post">
                          @csrf
                          <div class="iq-sub-card {{ !isset($selected_branch) ? 'bg-primary-subtle' : '' }} iq-branch-dropdown border-bottom">
                            <div class="d-flex align-items-center">
                              <span class="iq-media-group">
                                  <div class="icon iq-icon-box-3 rounded-pill">{{substr('A',0,1)}}</div>
                              </span>
                        ...
                        @isset($auth_user_branches)
                          @foreach ($auth_user_branches as $key => $branch)
                            ...
                            <form action="{{route('backend.set-current-branch', $branch->id)}}" method="post">
                              @csrf
                              <div class="iq-sub-card {{ isset($selected_branch) && $branch->id == $selected_branch->id ? 'bg-primary-subtle' : '' }} iq-branch-dropdown border-bottom">
                                <div class="d-flex align-items-center">
                                  <span class="iq-media-group">
                ...
                @endif
                  ...
                  <li class="nav-item dropdown">
                      ...
                      <ul class="dropdown-menu dropdown-menu-end">
                        <div class="dropdown-header bg-primary py-2 rounded mb-3">
                          <div class="d-flex gap-2">
                              ...
                              <div class="d-flex flex-column align-items-start">
                                  <h6 class="m-0 text-white">{{ Auth::user()->full_name ?? default_user_name() }}</h6>
                        ...
                        @role(['super admin'])
                        <li>
                          <a class="dropdown-item d-flex justify-content-between align-items-center" href="{{ route('index') }}">{{ __('messages.home') }}  </a>
                        </li>
                        @endrole
                        <li>
                     ...
                     @if(!auth()->user()->hasRole('super admin'))
                        <li>
                          <a class="dropdown-item d-flex justify-content-between align-items-center" href="{{ route('backend.billing-page') }}">{{ __('messages.billing') }}</a>
                        </li>
                        <li>
                          <a class="dropdown-item d-flex justify-content-between align-items-center" href="{{ route('backend.subscription_detail') }}">{{ __('messages.cancel_subscription') }}</a>
                        </li>
                        @endif
                        <li>
                            <a class="dropdown-item d-flex justify-content-between align-items-center" href="{{ route('backend.my-profile') }}#/change-password">{{ __('messages.change_password') }}</a>
                        </li>
                        @role('admin')
                        <li>
                        ...
                        @endrole
                          <li>
                              <a class="dropdown-item d-flex justify-content-between align-items-center" href="{{ route('logout') }}" id="logout-link">
