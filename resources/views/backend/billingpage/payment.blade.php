...
@section('title') {{ __('messages.pricing') }} @endsection
@section('content')
<section class="section-spacing-bottom">
    <div class="card">
    <div class="card-body">
        <a href="{{ route('backend.upgrade-plan') }}" class="d-flex align-items-center gap-2 mb-5 text-body" href="#">
                ...
                <form action="{{ route('backend.payment-process') }}" method="POST" id="payment-form">
                    ...
                    <div class="payment-card rounded-2 mb-4">
                        ...
                        <div class="form-group">
                            <input type="hidden" id="selected-plan-id" name="plan_id" value="{{$data['selected_plan']}}">
                            <input type="hidden" id="selected-price-id" name="total_price_amount" value="{{$data['total_amount']}}">


                            <select id="payment-method" name="payment_method" class="form-select select2" value="">
                    ...
                    <!-- Available Coupons -->
                    <div class="bg-body available-coupon-card rounded-2 my-4 p-3 {{ isset($data['promotions']) && $data['promotions']->isNotEmpty() ? '' : 'd-none' }}" id="promotional_section">
                        ...
                        <div class="row flex-nowrap coupan-list" id="promotional_id">
                            ...
                            <div class="col-md-6 mb-2">
                                <label for="radio1" class="form-check-label rounded coupons-card active d-flex justify-content-between  gap-3 p-3">
                                    <div class="d-flex align-items-center justify-content-between gap-3">
                                    <input class="form-check-input coupon-radio" type="radio" name="coupon_id" id="coupon_id_{{$promotion['id']}}" value="{{$promotion['id']}}">
                                            <div>
                                                @if($promotion['coupon']['discount_type']=='percent')
                                                <p class="mb-2 font-size-14 fw-medium">{{ __('frontend.get_extra') }}
                                                    {{$promotion['coupon']['discount_percentage']}}% off </p>
                                                @else
                                                <p class="mb-2 font-size-14 fw-medium">{{ __('frontend.get_extra') }}
                                                    {{\Currency::formatSuperadmin($promotion['coupon']['discount_amount'])}} off </p>
                                                @endif
                                                <div class="d-flex align-items-center gap-2">
                                                    <p class="mb-0 font-size-14 fw-medium">{{ __('frontend.use_code') }}:</p>
                    ...
                    <div class="d-flex justify-content-end align-items-center flex-wrap row-gap-2 column-gap-4">
                ...
                <div class="" id="promotional_id" >
                    ...
                    <form>
                        <ul class="list-inline m-0 coupons-inner">
                            ...
                            @foreach ($data['promotions'] as $key => $promotion)
                            <li>
                                <label for="radio2" class="form-check-label rounded coupons-card d-flex justify-content-between gap-3 p-3">
                                    <div class="d-flex align-items-center justify-content-between gap-3">
                                        <input class="form-check-input coupon-radio" type="radio" name="coupon_id" id="coupon_id_{{$promotion['id']}}" value="{{$promotion['id']}}">
                                        <div>
                                            @if($promotion['coupon']['discount_type'] == 'percent')
                                                <p class="mb-2 font-size-14 fw-medium">{{ __('frontend.get_extra') }} {{$promotion['coupon']['discount_percentage']}}% {{ __('frontend.off') }}</p>
                                            @else
                                                <p class="mb-2 font-size-14 fw-medium">{{ __('frontend.get_extra') }} {{ \Currency::formatSuperadmin($promotion['coupon']['discount_amount']) }} {{ __('frontend.off') }}</p>
                                            @endif
                                            <div class="d-flex align-items-center gap-2">
                                                <p class="mb-0 font-size-14 fw-medium">{{ __('frontend.use_code') }}:</p>
                                                <h6 class="mb-0"> {{$promotion['coupon']['coupon_code']}}</h6>
        ...
        <div class="modal-content section-background">
            <div class="modal-body modal-body-inner">
                ...
                <div class="mt-5 pt-2 d-flex gap-3 flex-wrap justify-content-center">
                    <button type="button" class="btn btn-primary" data-bs-dismiss="modal">{{__('frontend.pay')}}</button>
