        ...
        <div class="row">
          <div class="col-12 row">
                ...
                <div class="form-group col-md-12">
                  ...
                  <div class="btn-group w-100" role="group" aria-label="Basic example">
                    <template v-for="(item, index) in BRANCH_FOR_OPTIONS" :key="index">
                      ...
                      <label class="btn btn-check-label" :for="`${item.id}-for`">{{ item.text }}</label>
                    </template>
                  </div>
                </div>
              </div>
              ...
              <div class="form-group col-md-6">
                ...
                <div class="d-flex align-items-center gap-3 form-control" role="group" aria-label="Basic checkbox toggle button group">
...
const validationSchema = yup.object({
  name: yup.string()
    .required(t('messages.branch_name_required'))
  ...
  address: yup.object({
    ...
    city: yup.string()
      ...
      .required(t('messages.city_required')),
    latitude: yup.string()
    .required(t('messages.latitude_required'))
    .matches(/^(-?\d+(\.\d+)?)$/, 'Latitude must be a valid number'),
    longitude: yup.string()
    .required(t('messages.longitude_required'))
    ...
    state: yup.string()
      ...
      .required(t('messages.state_required')),
    country: yup.string().required(t('messages.country_required')),
