...
class QuickBookingsController extends Controller
{
    ...
    private function base62_decode_with_random($encoded, $length = 7) {
        ...
        // Remove the random suffix (last 2 characters in this case)
        $encodedBase62 = substr($encoded, 0, $length);  // Strip the suffix
    
        // Decode the Base62 string
        $decoded = 0;
        $strlen = strlen($encodedBase62);
        for ($i = 0; $i < $strlen; $i++) {
            $decoded = $decoded * $base + strpos($chars, $encodedBase62[$i]);
        }
    ...
    public function services_list(Request $request)
    {
        ...
        $items = Service::with(['branches' => function ($query) use ($data) {
