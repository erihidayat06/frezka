<template>
  <template v-if="bookingData">
    <div class="offcanvas-header">
      <h4>
        {{ $t('messages.sales') }}{{ bookingData.id }} <span class="text-capitalize">({{ bookingData.status }})</span>
      </h4>
    ...
    <div class="offcanvas-body">
        ...
        <div class="d-flex align-items-start gap-3 mb-2">
          ...
          <div class="flex-grow-1">
            <div class="gap-2">
              ...
              <p class="m-0">
                <small>{{ $t('messages.client_since') }} {{ moment(bookingData.user_created).format('MMMM YYYY') }}</small>
              </p>
            </div>
          </div>
        </div>
      ...
      <div class="form-group">
        <div v-for="(service, index) in bookingData.services" :key="index" class="py-2">
          <div class="d-flex flex-column gap-1">
            ...
            <p class="m-0">
              <label><i>{{ $t('messages.with') }}</i></label> <strong>{{ bookingData.employee_name }}</strong>
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-if="bookingData.products != null" class="form-group border-top">
        <div v-for="(product, index) in bookingData.products" :key="index" class="py-2">
          <div class="d-flex flex-column gap-1">
            ...
            <p class="m-0">
              <label><i>{{ $t('messages.Sold_By') }}</i></label> <strong>{{ bookingData.employee_name }}</strong>
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="border-top border-bottom py-3">
        ...
        <div class="d-flex justify-content-between align-items-center">
          <span>{{ $t('messages.Tip_amount') }}</span>
          <span
      ...
      <div class="py-3">
        <p><strong> {{ $t('messages.Booking_detail') }}</strong></p>
        <div>
          <h6><i class="fa-regular fa-clock"></i> {{ $t('messages.Booked_on') }} {{ bookingData.created_at }}</h6>
        </div>
        <div class="">
          <h6><i class="fa-regular fa-user"></i> {{ $t('messages.Booked_by') }} {{ bookingData.created_by_name }}</h6>
        </div>
        <div class="mt-4">
          <h6><i class="fa-regular fa-clock"></i> {{ $t('messages.Updated_on') }} {{ bookingData.updated_at }}</h6>
        </div>
        <div class="">
          <h6><i class="fa-regular fa-user"></i> {{ $t('messages.Updated_by') }} {{ bookingData.updated_by_name }}</h6>
        </div>
      </div>
    </div>
  </template>
  <template v-else>
    <div class="offcanvas-header">
      <div>
        <h4>{{ $t('messages.sales') }}0 <span class="text-capitalize">(___________)</span></h4>
      </div>
    </div>
  </template>
</template>
