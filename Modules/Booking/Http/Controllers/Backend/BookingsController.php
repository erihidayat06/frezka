...
class BookingsController extends Controller
{
    ...
    {
        // Page Title
        $this->module_title = __('booking.title');
    ...
    public function index()
    {
        ...
        return view('booking::backend.bookings.index', compact('module_action', 'statusList', 'date','module_title'));
    }
    ...
    public function index_list(Request $request)
    {
        ...
        $employees = User::bookingEmployeesList()
    ...
    public function datatable_view(Request $request)
    {
        ...
        $export_columns = [
            [
                'value' => 'id',
                'text' => __('booking.lbl_id'),
            ],
            [
                'value' => 'date',
                'text' => __('booking.lbl_date_time'),
            ],
            [
                'value' => 'customer',
            ...
            [
                ...
                'text' => __('booking.lbl_amount'),
            ],
            [
                'value' => 'service_duration',
                'text' => __('booking.lbl_duration'),
            ],
            [
                'value' => 'employee',
                'text' => __('booking.lbl_staff_name'),
            ],
            [
                'value' => 'services',
                'text' => __('booking.lbl_services'),
            ],
            [
                'value' => 'status',
                'text' => __('booking.lbl_status'),
            ],
            [
                'value' => 'payment_status',
                'text' => __('booking.lbl_payment_status'),
            ],
            [
                'value' => 'updated_at',
                'text' => __('booking.lbl_update_at'),
            ],
        ];
        $export_url = route('backend.bookings.export');

        return view('booking::backend.bookings.index_datatable', compact('module_action', 'filter', 'booking_status', 'export_import', 'export_columns', 'export_url','module_title'));
    }
    ...
    public function store(BookingRequest $request)
    {
        ...
        if($auth_user->currentSubscription) {
            $currentBookingsCount = Booking::withTrashed()->where('created_by', $auth_user->id)
            ->where('created_at', '>=', $auth_user->currentSubscription->start_date ) // Only count bookings after the plan purchase date
            ->count();

            if ($currentBookingsCount >= $auth_user->currentSubscription->max_appointment) {
                return response()->json(['message' =>  __('messages.booking_limit_exceeded'), 'status' => false], 422);
        ...
        try {
            $type = 'new_booking';
            $messageTemplate = __('messages.new_booking').'#[[booking_id]]';
    ...
    public function edit($id)
    {
        ...
        ->whereHas('branch', function ($query) use ($user_id) {
    ...
    public function updateStatus($id, Request $request)
    {
        ...
        switch ($status) {
            case 'check_in':
                ...
                $messageTemplate = '#[[booking_id]]'.__('messages.has_checkin');
                $notify_message = str_replace('[[booking_id]]', $id, $messageTemplate);
                break;
            case 'checkout':
                $notify_type = 'checkout_booking';
                $messageTemplate = '#[[booking_id]]'.__('messages.has_checkout');
                $notify_message = str_replace('[[booking_id]]', $id, $messageTemplate);
                break;
            case 'completed':
                $notify_type = 'complete_booking';
                $messageTemplate = '#[[booking_id]]'.__('messages.has_completed');
                $notify_message = str_replace('[[booking_id]]', $id, $messageTemplate);
                break;
            case 'cancelled':
    ...
    public function payment_create(Request $request)
    {
        ...
        $data = [
            ...
            'tax' => $taxes
                ...
                ->orWhere('module_type', 'services')->where('created_by','=',Auth::id())->where('status', 1)->get(),
    ...
    public function viewInvoice(Request $request)
    {
        ...
        $booking = Booking::with(['services', 'user', 'products', 'userCouponRedeem', 'packages','branch'])->where('status', 'completed')->find($request->id);
    ...
    public function downloadInvoice(Request $request)
    {
        ...
        $filename = __('messages.invoice').'_' . $request->id . '.pdf';
        // Prepare data for notification
        $data = $this->sendNotificationOnBookingUpdate('complete_booking', 'Notification message', $booking, false);
        if ($data === false) {
            return response()->json(['status' => false, 'message' => __('messages.failed_to_send_notification')], 500);
        }
        ...
        $view = view("mail.invoice-templates.template1", ['data' => $data['booking']])->render();
