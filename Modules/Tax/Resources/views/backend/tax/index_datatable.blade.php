...
<div class="card">
    <div class="card-body">
            ...
            <div>
                ...
                <x-backend.quick-action url="{{route('backend.tax.bulk_action')}}">
                    ...
                    <div class="select-status d-none quick-action-field" id="change-status-action">
                        <select name="status" class="form-select select2 p-2" id="status" style="width:100%">
            ...
            <x-slot name="toolbar">
                <div>
                    <div class="datatable-filter">
                        <select name="column_status" id="column_status" class="select2" data-filter="select" style="width: 100%">
                ...
                <div class="input-group flex-nowrap top-input-search">
                  ...
                  @hasPermission('add_tax')
                      <button type="button" class="btn btn-primary" data-crud-id="{{ 0 }}" title=""><i class="fas fa-plus-circle me-2"></i>{{ __('messages.new') }}</button>
                  @endhasPermission
              </x-slot>
            </x-backend.section-header>
        ...
        <div data-render="app">
            <tax-form-offcanvas create-title="{{('messages.new') }} {{ ($module_title) }}"
                edit-title="{{ ('messages.edit') }} {{ ($module_title) }}"></tax-form-offcanvas>
        </div>
    </div>

    <div data-render="app">
        <tax-form-offcanvas create-title="{{ ('messages.create') }} {{ ($module_title) }}" edit-title="{{ ('messages.edit') }} {{ ($module_title) }}"></tax-form-offcanvas>
    </div>
</div>
@endsection
