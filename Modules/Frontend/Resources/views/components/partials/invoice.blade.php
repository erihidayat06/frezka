...
<body>
  <div class="header">
    <div class="main-logo" style="float: left;">
        @php
            $logoPath = str_replace(config('app.url'), '', setting('logo'));
            $fullLogoPath = public_path($logoPath);
        @endphp

        @if(setting('logo') && file_exists($fullLogoPath))
            <img class="logo-mini img-fluid" src="{{ $fullLogoPath }}" height="30" alt="logo">
        @else
            <img class="logo-mini img-fluid" src="{{ public_path('img/logo/logo.png') }}" height="30" alt="logo">
        @endif
    </div>
    <div style="float: right;">
        <span>{{__('messages.invoice_date')}}<span class="text-black">{{ now()->format('d/m/Y') }}</span></span>
        <span style="margin-left: 12px;">{{__('messages.invoice_ID')}}<span class="text-black">#{{$data->id}}</span></span>
    </div>
  </div>
  ...
  <div class="header-content" style="border-bottom: 1px solid #CCCCCC; padding-bottom: 20px; margin-bottom: 20px;">
        ...
        <div class="right-content text-black" style="text-align: right; float: right; width: 30%;">
            <a href="mailto:customer@frezka.com" class="text-black">{{setting('inquriy_email') }}</a>
            <p style="margin: 10px 0 0;">{{setting('helpline_number') }}</p>
        </div>
        <div style="clear: both;"></div>
  </div>
    ...
    <div>
        ...
        <div style="margin-top: 10px; padding: 20px 24px; background-color: #FAF8FF;">
            <ul>
                <li><span class="text-black">{{__('messages.name:')}}</span>{{ $data->user->first_name . ' ' . $data->user->last_name }}</li>
                <li><span class="text-black">{{__('messages.email:')}}</span>{{$data->user->email}}</li>
                <li><span class="text-black">{{__('messages.mobile_number:')}}</span>{{$data->user->mobile}}</li>
            </ul>
        </div>
    </div>
    ...
    <div style="margin: 32px 0;">
        ...
        <div style="margin-top: 10px; padding: 20px 24px; background-color: #FAF8FF;">
            <ul>
                               <li><span class="text-black">{{__('messages.Name:')}}</span>{{ optional($data->plan)->name ?? '-' }}</li>
                <li><span class="text-black">{{__('messages.Duration:')}}</span>{{optional($data->plan)->duration. ' '.$displayType ?? '' }}</li>
                <li><span class="text-black">{{__('messages.purchase_date:')}}</span>{{ \Carbon\Carbon::parse($data->start_date)->format('d/m/Y') ?? "-" }}</li>
                <li><span class="text-black">{{__('messages.expiry_date:')}}</span>{{ \Carbon\Carbon::parse($data->end_date)->format('d/m/Y') ?? "-" }}</li>
                <li><span class="text-black">{{__('messages.Payment_method:')}}</span>{{ ucfirst(optional($data->subscription_transaction)->payment_type) }}</li>
    ...
    <table style="width: 100%; border-collapse: collapse; margin-top: 24px;">
        <tbody class="bg-color text-black">
            ...
            @php
            $discountData = optional($data->subscription_transaction)->discount_data ? json_decode($data->subscription_transaction->discount_data) : null;
            $couponCode = isset($discountData->coupon->coupon_code) ? '(' . $discountData->coupon->coupon_code . ')' : '';
        @endphp
            ...
            <tr>
                ...
                <td style="padding:12px 30px; text-align: end;"></td>
                <td style="padding:12px 30px; text-align: start; color: #6C757D;">{{__('messages.coupon_discount')}}<span style="color: #09954D;">{{ $couponCode }}</span></td>
                <td style="padding:12px 30px; text-align: right; color: #09954D;">-{{ \Currency::formatSuperadmin($data->discount_amount) }}</td>
            </tr>
            <tr>
                <td style="padding:12px 30px; text-align: start;"></td>
                <td style="padding:12px 30px; text-align: end;"></td>
                <td style="padding:12px 30px; text-align: end;"></td>
                <td style="padding:12px 30px; text-align: start; color: #6C757D;">{{__('messages.Subtotal')}}</td>
                <td style="padding:12px 30px; text-align: right; color: #050505;">{{ \Currency::formatSuperadmin($data->amount-$data->discount_amount) }}</td>
            </tr>
                    ...
                    @if(isset($data->plan->taxes) && $data->plan->taxes->isNotEmpty())
                        @foreach ($data->plan->taxes as $tax)
                            @if($tax->type == 'Percentage')
                                ({{ $tax->title }}: {{ $tax->value }}%)<br>
                            @else
                                ({{ $tax->title }}: {{ \Currency::formatSuperadmin($tax->value) }})<br>
                            @endif
                        @endforeach
                    @else
            ...
            <tr>
                ...
                <td style="padding:12px 30px; text-align: right; color: #050505; border-top:1px solid #ccc;">{{ \Currency::formatSuperadmin($data->total_amount ?? "-") }}</td>
            </tr>
        </tbody>
    </table>
  ...
  <div class="bottom-section" style="margin-top: 12px;">
    ...
    <p>{{__('messages.by_purchasing_a_subscription,')}}<a href="#" class="text-black" style="text-decoration: none; color: #A82D86;">support@frezka.com</a></p>
  </div>
