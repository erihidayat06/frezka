...
<section class="otp-section" style="background-image: url({{ asset('/img/frontend/register-page-pattern.png') }}); background-repeat: no-repeat; background-size: cover;">
                    ...
                    <div class="text-center">
                        ...
                        <div class="otp-wrapper">
                            <div class="mb-5">
                                ...
                                <p class="mb-0 font-size-14">{{__('messages.Check_your_email_and_enter_the_code_you_received.')}}</p>
                            </div>
                            <form action="{{ route('verify-otp') }}" method="POST" id="otpForm">
                                ...
                                <div class="d-flex align-items-center justify-content-center gap-md-4 gap-2">
                                    @for ($i = 0; $i < 6; $i++)
                                        <input type="text" name="otp[]" class="form-control rounded iq-otp-input text-center" maxlength="1" required />
                                    @endfor
                                </div>
                                ...
                                <div class="d-flex align-items-center justify-content-center flex-wrap gap-1 mt-3 px-3">
                                    ...
                                    <a href="#" class="text-primary font-size-14 fw-medium" id="resendOtp" style="pointer-events: none; opacity: 0.5;">{{__('messages.resend_OTP')}}</a>
                                </div>

                                <small id="resendMessage" class="text-success d-none mt-2">{{__('messages.OTP_has_been_resent!')}}</small>

                                <div class="mt-5 pt-3">
                                    <button type="submit" class="btn btn-secondary">{{__('messages.verify')}}</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    resendOtpButton.addEventListener("click", async (e) => {
        ...
        try {
            const response = await fetch("{{ route('resend-otp') }}", {
                ...
                body: JSON.stringify({ email: form.querySelector("input[name='user_email']").value }),
            });
